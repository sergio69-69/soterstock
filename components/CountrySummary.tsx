  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-4 bg-white rounded-[10px] shadow-sm border border-gray-100 p-5 border-l-4 border-l-accent hover:shadow-md transition-shadow cursor-pointer">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-heading font-bold text-xl text-primary">{country.name}</h2>
          <p className="text-sm text-gray-500">
            {serverCount} {serverCount === 1 ? 'servidor' : 'servidores'} &middot; {totalStock} unidades en stock
          </p>
        </div>
      </div>
      <div className="sm:ml-auto flex items-center gap-3">
        <span className="text-sm text-gray-600">
          Desde <span className="font-semibold text-primary">{priceRange}</span>
        </span>
        <svg
